import{initializeApp as P}from"https://www.gstatic.com/firebasejs/11.10.0/firebase-app.js";import{getAuth as L,onAuthStateChanged as M,signInWithEmailAndPassword as B,createUserWithEmailAndPassword as E}from"https://www.gstatic.com/firebasejs/11.10.0/firebase-auth.js";import{getFirestore as A,doc as C,collection as p,query as b,orderBy as v,onSnapshot as g,setDoc as I,serverTimestamp as h,addDoc as y}from"https://www.gstatic.com/firebasejs/11.10.0/firebase-firestore.js";(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))a(t);new MutationObserver(t=>{for(const n of t)if(n.type==="childList")for(const l of n.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&a(l)}).observe(document,{childList:!0,subtree:!0});function i(t){const n={};return t.integrity&&(n.integrity=t.integrity),t.referrerPolicy&&(n.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?n.credentials="include":t.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function a(t){if(t.ep)return;t.ep=!0;const n=i(t);fetch(t.href,n)}})();function T(){return setTimeout(()=>{const e=document.getElementById("mobile-menu-button"),s=document.getElementById("mobile-menu");e&&s&&e.addEventListener("click",()=>{s.classList.toggle("hidden")})},0),`
...
<section class="bg-white text-gray-800">
  ...
  `}function o(e,s,i=!1){return`
    <div class="flex items-center space-x-4 md:flex-col md:space-x-0 md:space-y-2 bg-transparent md:bg-purple-50 md:p-4 rounded-xl shadow-none md:shadow hover:md:shadow-lg transition duration-300">
        ${i?e:`<i class="${e} text-2xl md:text-4xl text-purple-600"></i>`}
  ...
  `}function F(){return`
        ...
        ${K()}
        ${q()}
    `}function J(){return setTimeout(()=>{document.body.addEventListener("click",()=>{location.hash="#/portfolio"},{once:!0});const e=document.getElementById("lottie-wave");e&&lottie.loadAnimation({container:e,renderer:"svg",loop:!0,autoplay:!0,path:"/animation.json"})},0),`
  ...
  `}function O(){const e={};function s(a){const t=document.getElementById(a),n=t.children.length;e[a]=(e[a]||0)+1,e[a]>=n&&(e[a]=0),t.style.transform=`translateX(-${e[a]*100}%)`}function i(a){const t=document.getElementById(a),n=t.children.length;e[a]=(e[a]||0)-1,e[a]<0&&(e[a]=n-1),t.style.transform=`translateX(-${e[a]*100}%)`}return window.nextSlide=s,window.prevSlide=i,setTimeout(()=>{document.querySelectorAll(".see-more-btn").forEach(a=>{a.addEventListener("click",()=>{const t=a.previousElementSibling;t.classList.contains("max-h-[140px]")?(t.classList.remove("max-h-[140px]"),t.classList.add("max-h-[1000px]"),a.textContent="Tampilkan lebih sedikit"):(t.classList.remove("max-h-[1000px]"),t.classList.add("max-h-[140px]"),a.textContent="Lihat selengkapnya")})})},0),`
    <section class="bg-white text-gray-800 py-20 px-6">
      <div class="max-w-7xl mx-auto text-center mb-12">
  ...
  `}function N(){return setTimeout(()=>{const e=document.getElementById("toggle-register"),s=document.getElementById("register-container");let i=!1;e?.addEventListener("click",()=>{i=!i,s.classList.toggle("max-h-0",!i),s.classList.toggle("max-h-screen",i)})},0),`
  ...
  `}const V={apiKey:"AIzaSyA2NgjgwjBogA5Capz7d5_PPsT3tkj9oTk",authDomain:"portfoliodimasforum.firebaseapp.com",projectId:"portfoliodimasforum",storageBucket:"portfoliodimasforum.appspot.com",messagingSenderId:"365108246554",appId:"1:365108246554:web:16f36847aaf7d0610eb9a3",measurementId:"G-NWM4FNMMK4"},k=P(V),f=L(k),m=A(k);function G(){const e=document.getElementById("chat-box"),s=document.getElementById("chat-form"),i=document.getElementById("message");M(f,a=>{if(!a)return window.location.hash="#/login";const t=C(m,"chats",a.uid),n=p(m,`chats/${a.uid}/messages`),l=b(n,v("timestamp","asc"));g(l,r=>{e.innerHTML="",r.forEach(d=>{const c=d.data();e.innerHTML+=`<div><strong>${c.sender==="admin"?"👨‍💼 Admin":"👤 You"}</strong>: ${c.text}</div>`,e.scrollTop=e.scrollHeight})}),s.addEventListener("submit",async r=>{r.preventDefault(),i.value.trim()&&(await I(t,{userEmail:a.email,createdAt:h()},{merge:!0}),await y(n,{text:i.value.trim(),sender:"user",timestamp:h()}),i.value="")})})}function W(){const e=document.getElementById("login-form"),s=document.getElementById("register-form"),i=document.getElementById("login-error"),a=document.getElementById("register-error");e&&e.addEventListener("submit",async t=>{t.preventDefault();const n=e.email.value.trim(),l=e.password.value.trim();try{await B(f,n,l),window.location.hash="#/forums"}catch{i.classList.remove("hidden")}}),s&&s.addEventListener("submit",async t=>{t.preventDefault();const n=s.email.value.trim(),l=s.password.value.trim(),r=s.confirm.value.trim();if(l.length<6){a.textContent="Password minimal 6 karakter",a.classList.remove("hidden");return}if(l!==r){a.textContent="Password tidak cocok",a.classList.remove("hidden");return}try{await E(f,n,l),window.location.hash="#/forums"}catch{a.textContent="Registrasi gagal. Gunakan email yang valid.",a.classList.remove("hidden")}})}function Z(){const e=document.getElementById("users"),s=document.getElementById("chat-box-admin"),i=document.getElementById("admin-chat-form"),a=document.getElementById("admin-message");let t=null;const n=p(m,"chats");g(n,r=>{e.innerHTML="",r.forEach(d=>{const c=d.data(),u=document.createElement("div");u.className="p-2 bg-white border rounded hover:bg-purple-100 cursor-pointer",u.textContent=c.userEmail,u.onclick=()=>l(d.id),e.appendChild(u)})});function l(r){t=r;const d=p(m,`chats/${r}/messages`),c=b(d,v("timestamp","asc"));g(c,u=>{s.innerHTML="",u.forEach(S=>{const x=S.data(),j=x.sender==="admin"?"👨‍💼 Admin":"👤 User";s.innerHTML+=`<div class="mb-2"><strong>${j}:</strong> ${x.text}</div>`}),s.scrollTop=s.scrollHeight})}i.addEventListener("submit",async r=>{if(r.preventDefault(),!t||!a.value.trim())return;const d=p(m,`chats/${t}/messages`);await y(d,{text:a.value.trim(),sender:"admin",timestamp:h()}),a.value=""})}const Q={"/":J,"/portfolio":F,"/portfolio/project":O,"/forums":z,"/login":N,"/admin":U};function w(){const e=document.getElementById("app"),s=location.hash.slice(1)||"/",i=Q[s]||(()=>"<h1>404 Page Not Found!</h1>"),a=s==="/"?i():$(i());e.innerHTML=a,s==="/forums"&&G(),s==="/login"&&W(),s==="/admin"&&Z()}window.addEventListener("DOMContentLoaded",w);window.addEventListener("hashchange",w);
