...
<section class="bg-white text-gray-800">
  ...
  `}function F(){return setTimeout(()=>{const e=document.getElementById("main-page");e&&e.classList.add("opacity-100","translate-y-0")},50),`
  ...
  `}function O(){const e={};function s(a){const t=document.getElementById(a),i=t.children.length;e[a]=(e[a]||0)+1,e[a]>=i&&(e[a]=0),t.style.transform=`translateX(-${e[a]*100}%)`}function n(a){const t=document.getElementById(a),i=t.children.length;e[a]=(e[a]||0)-1,e[a]<0&&(e[a]=i-1),t.style.transform=`translateX(-${e[a]*100}%)`}return window.nextSlide=s,window.prevSlide=n,setTimeout(()=>{document.querySelectorAll(".see-more-btn").forEach(a=>{a.addEventListener("click",()=>{const t=a.previousElementSibling;t.classList.contains("max-h-[140px]")?(t.classList.remove("max-h-[140px]"),t.classList.add("max-h-[1000px]"),a.textContent="Tampilkan lebih sedikit"):(t.classList.remove("max-h-[1000px]"),t.classList.add("max-h-[140px]"),a.textContent="Lihat selengkapnya")})})},0),setTimeout(()=>{const a=document.getElementById("portfolio-section");a&&a.classList.add("opacity-100")},50),`
    <section id="portfolio-section" class="opacity-0 transition-opacity duration-700 bg-white text-gray-800 py-20 px-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
        ...
        <div class="bg-purple-50 rounded-xl shadow hover:shadow-lg transition duration-300 overflow-hidden">
          <img src="/myPortfolio/project/absensi.png" alt="Project Absensi Staff" class="w-full aspect-[16/9] object-cover">
          <div class="p-6">
            <h3 class="text-lg font-semibold text-purple-800 mb-2">Absensi Staff</h3>
            <div class="relative">
              <p class="desc text-sm text-gray-600 overflow-hidden max-h-[140px] transition-all duration-300">
                Aplikasi absensi berbasis web yang dirancang untuk mencatat kehadiran karyawan secara efisien dan akurat. Sistem ini menggunakan verifikasi lokasi (GPS) dan kamera (foto wajah) untuk memastikan absensi dilakukan di tempat yang sesuai. Fitur lain termasuk pengajuan absen manual, riwayat kehadiran, dan dashboard admin.
                <br><br>
                Dibangun dengan React, Tailwind CSS, dan Laravel.
              </p>
              <button class="see-more-btn text-purple-600 text-sm mt-2 hover:underline">Lihat selengkapnya</button>
            </div>
          </div>
